...
var view='login',matchIdx=0,authMode='login';
var comments={};
...
function tplLogin(){
  return'<section class="screen login-screen"><div class="hero"><h1 class="logo-lockup"><span class="logo-badge">\u{1f3c6}</span><span class="logo-anime"><i>\u52a8</i><i>\u6f2b</i><i>\u4e4b</i><i>\u5dc5</i></span><span class="logo-cup">CUP</span></h1><p class="slogan">\u51b3\u6218\u52a8\u6f2b\u4e4b\u5dc5</p><p class="tagline">32\u90e8\u52a8\u6f2b\u6dd8\u6c70\u8d5b \u00b7 \u9009\u51fa\u4f60\u7684\u672c\u547d</p></div><div class="auth-box"><div class="auth-tabs"><button class="auth-tab active" data-act="auth-tab" data-tab="login">\u767b\u5f55</button><button class="auth-tab" data-act="auth-tab" data-tab="register">\u6ce8\u518c</button></div><div id="authForm">'+tplAuthForm('login')+'</div><p class="auth-error" id="authError" hidden></p></div><div class="home-foot"><button class="btn ghost sm" data-act="show-results">\u67e5\u770b\u5b9e\u65f6\u6392\u540d</button></div></section>';
}
function tplAuthForm(mode){
  return'<div class="auth-fields"><input id="authEmail" type="email" placeholder="\u90ae\u7bb1\u5730\u5740"><input id="authPass" type="password" placeholder="\u5bc6\u7801\uff08\u81f3\u5c116\u4f4d\uff09" autocomplete="'+(mode==='login'?'current-password':'new-password')+'"><button class="btn primary" data-act="'+(mode==='login'?'login':'register')+'">'+(mode==='login'?'\u767b\u5f55':'\u6ce8\u518c')+'</button></div>';
}
function tplVoting(){
...
(async function(){var s=await SB.auth.getSession();if(s.data.session){user=s.data.session.user;try{await loadAnimes();buildBracket();await loadMyVotes();view='voting';}catch(e){view='login';if(e.message==='need_32')AC.toast('\u540e\u53f0\u8fd8\u672a\u8bbe\u7f6e32\u5f3a\u52a8\u6f2b');}}render();})();
